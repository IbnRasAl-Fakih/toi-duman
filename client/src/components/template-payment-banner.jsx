...
export default function TemplatePaymentBanner({ order }) {
  ...
  return (
    <div className="sticky inset-x-0 top-0 z-50 border-b border-black/10 bg-[rgba(20,18,18,0.82)] text-[#f8f2ec] shadow-[0_12px_32px_rgba(0,0,0,0.18)] backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-3 px-4 py-3 md:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-10">
        ...
        <div className="flex flex-col gap-2 md:min-w-[300px]">
          {order ? (
            <div className="grid gap-1.5 rounded-[18px] border border-white/10 bg-white/6 px-4 py-2.5 text-sm text-[#f8f2ec]/88 backdrop-blur-md">
          ...
          <Link
            to={paymentPath}
            className="inline-flex items-center justify-center rounded-full bg-[#7f1118] px-6 py-2.5 text-[11px] uppercase tracking-[0.16em] text-white shadow-[0_10px_24px_rgba(127,17,24,0.22)] transition hover:bg-[#5d0b11]"
          >
            Перейти к оплате
          </Link>
        </div>
...
function InfoRow({ label, value }) {
  return (
    <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-1.5 last:border-b-0 last:pb-0">
      <span className="text-[10px] uppercase tracking-[0.22em] text-[#f8f2ec]/58">{label}</span>
