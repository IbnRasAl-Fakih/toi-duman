  ...
  return (
    <button
      ...
      className={`rounded-full border px-4 py-3 text-left text-sm transition ${
        isSelected ? "border-white bg-white text-[#7a0626]" : "border-white/30 bg-transparent text-white hover:border-white/60 hover:bg-white/10"
      }`}
    >
      {option.label}
    </button>
    ...
    <CeremonialRevealSection className="relative bg-[#66021F] text-white">
      <div className="px-7 pb-0 pt-11 text-center">
        ...
        <CeremonialRevealItem delay={220} className="mx-auto mt-9 grid max-w-[320px] gap-3 text-left">
          ...
          {isTemplateElementEnabled(template, "rsvp.options") ? (
        ...
        {isTemplateElementEnabled(template, "rsvp.farewell") ? (
      ...
      {isTemplateElementEnabled(template, "rsvp.farewellImage") ? (
        <CeremonialRevealItem delay={380} className="relative mt-10">
          <img src={template.farewell.imageUrl} alt={template.farewell.imageAlt} className="block w-full" />
