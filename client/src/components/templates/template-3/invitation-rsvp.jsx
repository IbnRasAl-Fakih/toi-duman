...
export default function InvitationRsvpTemplate3({
  ...
  return (
    <section className="relative overflow-hidden text-white">
      ...
      <div className="relative px-10 pb-0 pt-24">
        ...
        <div className="mt-12 space-y-5 font-['Segoe_UI','Arial',sans-serif]">
          <label className="block">
            ...
            <input
              ...
              className="h-12 w-full rounded-[4px] border border-white/35 bg-white px-4 text-[1rem] text-[#1e1e1e] outline-none placeholder:text-black/25 focus:border-white disabled:cursor-not-allowed disabled:opacity-50"
            />
          </label>
          ...
          <div>
            ...
            <div className="mt-3 space-y-2.5">
              {responseOptions.map((option) => (
                <label key={option.value} className={`flex items-center gap-2.5 text-[0.95rem] font-semibold ${isDisabled ? "opacity-50" : ""}`}>
                  <input
                    type="radio"
                    ...
                    disabled={isDisabled}
                    onChange={() => (isDisabled ? null : onSelectStatus(option.value))}
